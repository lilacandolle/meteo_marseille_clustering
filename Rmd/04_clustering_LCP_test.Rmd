...
```{r}
# pour plage tout
hourly_matin_DJF_acp <- hourly_matin_DJF %>%
  select(all_of(vars_clustering)) %>%
  scale(center = TRUE, scale = TRUE) %>%
  prcomp()

# pour plage vent faible
hourly_matin_DJF_ventfaible_acp <- hourly_matin_DJF_ventfaible %>%
  select(all_of(vars_clustering)) %>%
  scale(center = TRUE, scale = TRUE) %>%
  prcomp()

# pour plage tout
plage_matin_DJF_acp <- plage_matin_DJF %>%
  select(all_of(vars_clustering)) %>%
  scale(center = TRUE, scale = TRUE) %>%
  prcomp()

# pour plage vent faible
plage_matin_DJF_ventfaible_acp <- plage_matin_DJF_ventfaible %>%
  select(all_of(vars_clustering)) %>%
  scale(center = TRUE, scale = TRUE) %>%
  prcomp()
...
plot_clustering_results(plage_matin_DJF, 'completeCAH', 'complete_CAH', 'plage_matin_DJF_complete_CAH', vars_dict = vars_dict)
```
...
plot_clustering_results(hourly_matin_DJF, 'GMM', 'GMM_hourly_matin_DJF', 'GMM_hourly_matin_DJF', vars_dict = vars_dict)1
```
